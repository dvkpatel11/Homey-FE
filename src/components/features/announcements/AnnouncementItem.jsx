  ...
  return (
    <Card className="p-8">
      <div className="flex items-start space-x-4">
        <div className={`w-12 h-12 bg-gradient-to-br ${isDarkMode ? 'from-indigo-400 to-purple-500' : 'from-indigo-500 to-purple-600'} rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0`}>
        ...
        <div className="flex-1">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <span className={`text-sm ${isDarkMode ? 'bg-gray-700/60 text-gray-200' : 'bg-gray-200/50 text-gray-500'} px-3 py-1 rounded-full font-light backdrop-blur-lg`}>
          ...
          {announcement.type === 'poll' && (
            <div className={`${themeClasses.cardGlass} rounded-2xl p-6 mb-6`}>
              <div className="grid grid-cols-2 gap-4">
                <Button
                  variant="success"
                  onClick={() => onVote(announcement.id, 'yes')}
                  className="py-3"
                >
                ...
                <Button
                  variant="danger"
                  onClick={() => onVote(announcement.id, 'no')}
                  className="py-3"
                >
            ...
            <div className="space-y-3 mb-6">
              {announcement.comments.map((comment, commentIndex) => (
                <div key={commentIndex} className={`${themeClasses.cardGlass} rounded-2xl p-4`}>
                  ...
                  <p className={`${isDarkMode ? 'text-gray-200' : 'text-gray-700'} font-light`}>
                    {comment.content}
                  </p>
                </div>
              ))}
