    ...
    <Card className="p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h3 className={`font-medium text-lg ${task.completed ? `line-through ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}` : `${isDarkMode ? 'text-white' : 'text-gray-900'}`}`}>
              {task.title}
            </h3>
            <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'} font-light`}>
              {task.assignee} • Due {task.dueDate}
            </p>
            <div className={`inline-block px-3 py-1 rounded-full text-xs font-medium mt-2 ${getPriorityColor(task.priority, isDarkMode)}`}>
