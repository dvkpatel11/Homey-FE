...
export const useRealtime = () => {
              ...
              queryClient.setQueryData(["households", householdId, "messages"], (old) => {
                ...
                return {
                  ...
                  pages: old.pages.map((page) => ({
                    ...page,
                    data: page.data.map((msg) => (msg.id === newMessage.id ? newMessage : msg)),
                  })),
                };
              });
              break;
  ...
  const subscribeToExpenseUpdates = useCallback(
    (householdId) => {
      if (!householdId) return;
  ...
  return {
    isConnected,
    connectionError,
    isEnabled: import.meta.env.VITE_REALTIME_ENABLED === "true",
